...
public class DBScanService extends Service {
    ...
    private final int radius = 30;//聚类半径
    ...
    private void DBScanAnalysis()
    {
        ...
        try{
            ...
            eachTypeTotalPts = new int[typeNum+1];//..Pts[i]表示类别为i的点数量,多出来的1为其它点（未参加聚类）
            eachTypeDuraTime = new int[typeNum+1];//..Time[i]表示类别i的轨迹时间,多出来的1为其它点（未参加聚类）
            ...
            for(LocPoint point : points)
            {
                ...
                eachTypeTotalPts[point.getCluster()]++;//统计每个类别的点数量
                eachTypeDuraTime[point.getCluster()]+=getDurationTime(point);//统计每个类别的总时间
            }
        }catch (Exception e)
        {
